...
<?php
  include_once 'connection.php';
  $edit_error = false;
  $edit_student_username = $_SESSION['usr_id'];

  if(isset($_POST['savechanges'])) {
    $edit_name = mysqli_real_escape_string($con, $_POST['edit_name']);
    $edit_dob = $_POST['edit_dob'];
    $edit_mobileno = mysqli_real_escape_string($con, $_POST['edit_mobileno']);
    $edit_password = mysqli_real_escape_string($con, $_POST['edit_password']);

    if (!preg_match("/^[a-zA-Z ]+$/",$edit_name)) {
        $edit_error = true;
        $edit_name_error = "Name must contain only alphabets and space";
    }
    if(strlen($edit_password) < 6) {
        $edit_error = true;
        $edit_password_error = "Password must be minimum of 6 characters";
    }
    if (!$edit_error) {
        if(mysqli_query($con, "UPDATE `student_information` SET `student_name`= $edit_name,`student_date_of_birth`= $edit_dob,`student_mobileno`=$edit_mobileno,`student_password`=$edit_password  WHERE `student_username` = $edit_student_username")) {
            $edit_successmsg = "Successfully Updated!!";
        } else {
            $edit_errormsg = "Error in Updating Profile...Please try again later!";
        }
    }
  }
?>
...
<html lang="en">
    ...
    <div id="editprofile" class="container">
        ...
        <form role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" name="editform">
          <div class="form-group">
            <label for="usrname"><span class="glyphicon glyphicon-user"></span> Name</label>
            <input type="text" name = "edit_name" required value="<?php if($edit_error) echo $name; ?>" class="form-control" id="fname" placeholder="Name">
              <span class="text-danger"><?php if (isset($edit_name_error)) echo $edit_name_error; ?></span>
          </div>
          <div class="form-group">
            <label for="dte"><span class="glyphicon glyphicon-user"></span> Date of Birth</label>
            <input type="date" name = "edit_dob" required class="form-control" id="dob" placeholder="Date of birth">
          </div>
          <div class="form-group">
            <label for="usrname"><span class="glyphicon glyphicon-user"></span> Mobile no</label>
            <input type="text" name = "edit_mobileno" required class="form-control" id="mobileno" placeholder="Mobile no">
          </div>
          <div class="form-group">
            <label for="usrname"><span class="glyphicon glyphicon-lock"></span> Change password</label>
            <input type="password" name = "edit_password" required class="form-control" id="pwd" placeholder="Enter password">
            <span class="text-danger"><?php if (isset($edit_password_error)) echo $edit_password_error; ?></span>
          </div>
          <button type="submit" name = "savechanges" class="btn btn-block">SAVE CHANGES 
            <span class="glyphicon glyphicon-ok"></span>
          </button>
        </form>
        <span class="text-success"><?php if (isset($edit_successmsg)) { echo $edit_successmsg; } ?></span>
        <span class="text-danger"><?php if (isset($edit_errormsg)) { echo $edit_errormsg; } ?></span>
    </div>
			<!-- Modal -->
